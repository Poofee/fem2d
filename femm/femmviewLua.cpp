...
void CFemmviewDoc::initalise_lua()
{
	...
	lua_register(lua,"mo_elements",lua_elements);
	lua_register(lua,"mo_nodes",lua_nodes);
	lua_register(lua,"mo_blocks",lua_lbls);
	lua_register(lua,"mo_materials",lua_blks);
	lua_register(lua,"mo_getelement",lua_getelement);
	lua_register(lua,"mo_getnode",lua_getnode);
	lua_register(lua,"mo_getblock",lua_getlbl);
	lua_register(lua,"mo_getmaterial",lua_getblk);
	lua_register(lua,"mo_get_element",lua_getelement);
	lua_register(lua,"mo_get_node",lua_getnode);
	lua_register(lua,"mo_get_block",lua_getlbl);
	lua_register(lua,"mo_get_material",lua_getblk);
...
int CFemmviewDoc::lua_elements(lua_State *L)
{
	CatchNullDocument();
	CFemmviewDoc * thisDoc;
	thisDoc=(CFemmviewDoc *)pFemmviewdoc;
...
int CFemmviewDoc::lua_nodes(lua_State *L)
{
	CatchNullDocument();
	CFemmviewDoc * thisDoc;
	thisDoc=(CFemmviewDoc *)pFemmviewdoc;
...
int CFemmviewDoc::lua_lbls(lua_State *L)
{
	CatchNullDocument();
	CFemmviewDoc * thisDoc;
	thisDoc=(CFemmviewDoc *)pFemmviewdoc;

	lua_pushnumber(L,thisDoc->blocklist.GetSize());

	return 1;
}

int CFemmviewDoc::lua_blks(lua_State *L)
{
	CatchNullDocument();
	CFemmviewDoc * thisDoc;
	thisDoc=(CFemmviewDoc *)pFemmviewdoc;

	lua_pushnumber(L,thisDoc->blockproplist.GetSize());
...
int CFemmviewDoc::lua_getelement(lua_State *L)
{
	...
	for (j=0;j<3;j++) lua_pushnumber(L,thisDoc->meshelem[k].p[j]);
	lua_pushnumber(L,thisDoc->meshelem[k].blk);
	lua_pushnumber(L,thisDoc->meshelem[k].lbl);
	lua_pushnumber(L,thisDoc->meshelem[k].B1);	
	lua_pushnumber(L,thisDoc->meshelem[k].B2);
	for (j=0;j<3;j++) lua_pushnumber(L,thisDoc->meshelem[k].b1[j]);
	for (j=0;j<3;j++) lua_pushnumber(L,thisDoc->meshelem[k].b2[j]);
	lua_pushnumber(L,thisDoc->meshelem[k].magdir);
...
int CFemmviewDoc::lua_getnode(lua_State *L)
{
	...
	lua_pushnumber(L,thisDoc->meshnode[k].A);
	return 3;
}

int CFemmviewDoc::lua_getlbl(lua_State *L)
{
	CatchNullDocument();
	CFemmviewDoc * thisDoc;
	thisDoc=(CFemmviewDoc *)pFemmviewdoc;
	...
	k=(int) lua_todouble(L,1);
	if ((k<0) || (k>=thisDoc->blocklist.GetSize())) return 0;

/*
	// block label data elements
	double x,y;
	double MaxArea;
	double MagDir;
	CString MagDirFctn;
	...
	CComplex o,mu;
*/
	lua_pushnumber(L,thisDoc->blocklist[k].x);
	lua_pushnumber(L,thisDoc->blocklist[k].y);
	lua_pushnumber(L,thisDoc->blocklist[k].BlockType);
	lua_pushnumber(L,thisDoc->blocklist[k].MaxArea);
	lua_pushnumber(L,thisDoc->blocklist[k].InCircuit);
	lua_pushnumber(L,thisDoc->blocklist[k].InGroup);
	lua_pushnumber(L,thisDoc->blocklist[k].Turns);
	lua_pushnumber(L,thisDoc->blocklist[k].IsExternal);
	if (thisDoc->blocklist[k].MagDirFctn.GetLength()>0)
		lua_pushstring(L,thisDoc->blocklist[k].MagDirFctn);
	else
		lua_pushnumber(L,thisDoc->blocklist[k].MagDir);

	return 9;
}

int CFemmviewDoc::lua_getblk(lua_State *L)
{
	CatchNullDocument();
	CFemmviewDoc * thisDoc;
	thisDoc=(CFemmviewDoc *)pFemmviewdoc;
	...
	k=(int) lua_todouble(L,1);
	if ((k<0) || (k>=thisDoc->blockproplist.GetSize())) return 0;
...
*/
	...
	lua_pushstring(L,thisDoc->blockproplist[k].BlockName);
