...
void CMyCommandLineInfo::ParseParam (LPCTSTR lpszParam,BOOL bFlag,BOOL bLast )
{
	...
	if (theparam=="windowhide" && bFlag==1) m_luaWindowStatus=SW_MINIMIZE;
	if (theparam=="hidewindow" && bFlag==1) m_luaWindowStatus=SW_MINIMIZE;

	if (theparam=="filelink") ((CFemmApp *)AfxGetApp())->bFileLink=TRUE;
