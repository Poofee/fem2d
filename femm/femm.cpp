...
//	if(AfxOleRegisterTypeLib(AfxGetInstanceHandle(), LIBID_ActiveFEMM, _T("femm.TLB"))==FALSE) AfxMessageBox("TypeLib not registered!"); 
...
void CFemmApp::OnFileOpenLuaScript() 
{
	...
	if(MyDlg.DoModal()==IDOK) luascriptname=MyDlg.GetPathName();
}
...
void CFemmApp::CreateNewDocument(int n)
{
	...
	{	
		...
		sprintf(CommandLine,"%sfemmplot.exe",((CFemmApp *)AfxGetApp())->GetExecutablePath());
...
void CFemmApp::MOCheckIfAlreadyOpen(CString p)
{	
	POSITION tpos,dpos;
	CDocTemplate* pTemp;
	CFemmviewDoc* pDoc;
...
void CFemmApp::EOCheckIfAlreadyOpen(CString p)
{
	POSITION tpos,dpos;
	CDocTemplate* pTemp;
	CbelaviewDoc* pDoc;
...
void CFemmApp::HOCheckIfAlreadyOpen(CString p)
{
	POSITION tpos,dpos;
	CDocTemplate* pTemp;
	ChviewDoc* pDoc;
...
void CFemmApp::COCheckIfAlreadyOpen(CString p)
{
	POSITION tpos,dpos;
	CDocTemplate* pTemp;
	CcviewDoc* pDoc;
