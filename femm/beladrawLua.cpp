...
int CbeladrawDoc::lua_readdxf(lua_State *L)
{
	...
	DocTitle.Format("%s",lua_tostring(L,n));
	if (n==2) DefTol=lua_todouble(L,1);
	else DefTol=0;
...
PBITMAPINFO CbeladrawDoc::CreateBitmapInfoStruct(HWND hwnd, HBITMAP hBmp)
{
    ...
    if (!GetObject(hBmp, sizeof(BITMAP), (LPSTR)&bmp))
        ...
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy1");
        }
...
void CbeladrawDoc::CreateBMPFile(HWND hwnd, LPTSTR pszFile, PBITMAPINFO pbi,
                  HBITMAP hBMP, HDC hDC)
 {
    ...
    if (!lpBits)
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy2");
//              errhandler("GlobalAlloc", hwnd);
    ...
    //    errhandler("GetDIBits", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy3");
    ...
    {//    errhandler("CreateFile", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy4");
    ...
    if (!WriteFile(hf, (LPVOID) &hdr, sizeof(BITMAPFILEHEADER),
        (LPDWORD) &dwTmp,  NULL))
    {
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy5");
    ...
    if (!WriteFile(hf, (LPVOID) pbih, sizeof(BITMAPINFOHEADER)
                  ...
                  (LPDWORD) &dwTmp, ( NULL)))
//        errhandler("WriteFile", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy6");
...
//           errhandler("WriteFile", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy7");

    // Close the .BMP file.
     if (!CloseHandle(hf))
//           errhandler("CloseHandle", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy8");
