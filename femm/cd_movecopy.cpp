// cdrawDoc.cpp : implementation of the CcdrawDoc class (continued...)
//
...
BOOL CcdrawDoc::ReadDXF(CString fname,double DefTol)
{
	FILE *fp;
	int j,k;
...
void CcdrawDoc::RotateCopy(CComplex c, double dt, int ncopies, int EditAction)
{
  int i,j,k,nc;
  CComplex x,z;
  double t;
...
void CcdrawDoc::TranslateCopy(double incx, double incy, int ncopies, int EditAction)
{
  int i,j,k,nc;
  double dx,dy;
  for(nc=0;nc<ncopies;nc++){
...
void CcdrawDoc::ScaleMove(double bx, double by, double sf, int EditAction)
{
	int i;
...
void CcdrawDoc::MirrorSelected(double x0, double y0, double x1, double y1, int EditAction)
{
	int i,j,k;
	CComplex x,p,y;
