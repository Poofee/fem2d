...
BOOL CbelaviewDoc::MakeMask()
{
	...
	if(ProblemType==1)
	{
		...
		if (!bOnAxis) for(i=0;i<NumNodes;i++) 
	...
	for(i=0;i<blockproplist.GetSize();i++)
	{
		...
		k=0;
		if((blockproplist[i].ex!=1) || (blockproplist[i].ey!=1)) k=1;	
		if(blockproplist[i].qv!=0) k=1;
		matflag[i]=k;
	}
	...
	for(i=0;i<NumNodes;i++) if (meshnode[i].IsSelected==TRUE) L.V[i]=1;
...
BOOL CbelaviewDoc::IsKosher(int k)
{
    // If:
