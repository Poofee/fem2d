...
cdCLibDlg::cdCLibDlg(CWnd* pParent /*=NULL*/)
	: CResizableDialog(cdCLibDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(cdCLibDlg)
	//}}AFX_DATA_INIT
	// Note that LoadIcon does not require a subsequent DestroyIcon in Win32
	// m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void cdCLibDlg::DoDataExchange(CDataExchange* pDX)
{
	CResizableDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(cdCLibDlg)
	DDX_Control(pDX, IDC_MYLIST, m_mylist);
	DDX_Control(pDX, IDC_MYTREE, m_mytree);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(cdCLibDlg, CResizableDialog)
	//{{AFX_MSG_MAP(cdCLibDlg)
	ON_WM_QUERYDRAGICON()
...
/////////////////////////////////////////////////////////////////////////////
// cdCLibDlg message handlers

BOOL cdCLibDlg::OnInitDialog()
{
	...
	CString LibName=BinDir+"condlib.dat";
	FILE *fp;
	CMaterialProp MProp;
	CFolderProp FProp;
	char s[1024];
...
HCURSOR cdCLibDlg::OnQueryDragIcon()
{
...
char* cdCLibDlg::StripKey(char *c)
{
	char *d;
	int i,k;
...
int cdCLibDlg::ParseLine(char *s, FILE *fp, CMaterialProp &MProp)
{
		char q[1024];
		char *v;
		int i,k;
		...
		if( _strnicmp(q,"<ltx>",5)==0){
		   v=StripKey(s);
		   sscanf(v,"%lf",&MProp.ltx);
		   q[0]=NULL;
		}
		
		if( _strnicmp(q,"<lty>",5)==0){
		   v=StripKey(s);
		   sscanf(v,"%lf",&MProp.lty);
		   q[0]=NULL;
		}	
...
void cdCLibDlg::OnBegindragMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void cdCLibDlg::OnBegindragMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_TREEVIEW* pNMTreeView = (NM_TREEVIEW*)pNMHDR;
	m_dragItem = pNMTreeView->itemNew.hItem;
...
void cdCLibDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void cdCLibDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if( m_bIsDragging != FALSE )
    {
		KillTimer ( 1 ) ;
...
BOOL cdCLibDlg::IsFolder(HTREEITEM hItem)
{
	int nImage1, nImage2, nSelImage;		
	m_mytree.GetItemImage(hItem,nImage1,nSelImage);
	m_mylist.GetItemImage(hItem,nImage2,nSelImage);	
	if ((nImage1!=2) && (nImage2!=2)) return TRUE;
	return FALSE;
}

HTREEITEM cdCLibDlg::CopyItemToLibrary(HTREEITEM hItem, HTREEITEM hTarget)
{
	...
	if(!bDragFromLibrary)
	{
		...
		ItemData = m_mylist.GetItemData(hItem);
	}
	else{
		szLabel = m_mytree.GetItemText(hItem);
		ItemData = m_mytree.GetItemData(hItem);
	}
...
HTREEITEM cdCLibDlg::CopyItemToModel(HTREEITEM hItem, HTREEITEM hTarget)
{
	HTREEITEM hResult,hParent,hChild;
...
void cdCLibDlg::OnRclickMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
		CMenu* pMenu = NULL;
		try
		{
		...
		{
			ASSERT(FALSE);
			TRACE("cdCLibDlg::OnRclickMytree failed to destroy menu resource\n");
			pMenu = FALSE;
		}
...
void cdCLibDlg::OnRclickMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
		CMenu* pMenu = NULL;
		try
		{
		...
		{
			ASSERT(FALSE);
			TRACE("cdCLibDlg::OnRclickMylist failed to destroy menu resource\n");
			pMenu = FALSE;
		}
...
void cdCLibDlg::OnDblclkMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	POINT point;
	GetCursorPos(&point);
	CPoint ptList(point);
...
void cdCLibDlg::OnDblclkMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	POINT point;
	GetCursorPos(&point);
	CPoint ptList(point);
...
void cdCLibDlg::ModifyMaterial(int k) 
{
	cdCMatDlg zDlg;

	zDlg.m_ex=LibProps[k].ex;
	zDlg.m_ey=LibProps[k].ey;
	zDlg.m_ltx=LibProps[k].ltx;
	zDlg.m_lty=LibProps[k].lty;
	zDlg.m_ox=LibProps[k].ox;
	zDlg.m_oy=LibProps[k].oy;
...
//		if(nn!=k) zDlg.namelist.Add(LibProps[nn].BlockName);
	...
	if(zDlg.DoModal()==IDOK){
		...
		LibProps[k].ex=zDlg.m_ex;
		LibProps[k].ey=zDlg.m_ey;
		LibProps[k].ltx=zDlg.m_ltx;
		LibProps[k].lty=zDlg.m_lty;
		LibProps[k].ox=zDlg.m_ox;
		LibProps[k].oy=zDlg.m_oy;
	}
}

void cdCLibDlg::MouseModify()
{
	if(m_dragTargetList!=NULL)
	{
		if(!IsFolder(m_dragTargetList)){
...
void cdCLibDlg::CopyItemToDisk(HTREEITEM hItem, FILE *fp)
{
	HTREEITEM hChild;
...
void cdCLibDlg::ImportMaterials()
{
...
BOOL cdCLibDlg::IsTailChasing(HTREEITEM hSource, HTREEITEM hDest)
{
	HTREEITEM hItem;
...
void cdCLibDlg::OnKeydownMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void cdCLibDlg::OnKeydownMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	TV_KEYDOWN* pTVKeyDown = (TV_KEYDOWN*)pNMHDR;
...
void cdCLibDlg::OnTimer(UINT nIDEvent)
{
	POINT pt ;
    GetCursorPos ( &pt ) ;
    RECT rect ;
