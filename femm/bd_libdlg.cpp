...
bd_CLibDlg::bd_CLibDlg(CWnd* pParent /*=NULL*/)
	: CResizableDialog(bd_CLibDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(bd_CLibDlg)
	//}}AFX_DATA_INIT
	// Note that LoadIcon does not require a subsequent DestroyIcon in Win32
	// m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void bd_CLibDlg::DoDataExchange(CDataExchange* pDX)
{
	CResizableDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(bd_CLibDlg)
	DDX_Control(pDX, IDC_MYLIST, m_mylist);
	DDX_Control(pDX, IDC_MYTREE, m_mytree);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(bd_CLibDlg, CResizableDialog)
	//{{AFX_MSG_MAP(bd_CLibDlg)
	ON_WM_QUERYDRAGICON()
...
/////////////////////////////////////////////////////////////////////////////
// bd_CLibDlg message handlers

BOOL bd_CLibDlg::OnInitDialog()
{
	...
	CString LibName=BinDir+"statlib.dat";
	FILE *fp;
	CMaterialProp MProp;
	CFolderProp FProp;
	char s[1024];
...
HCURSOR bd_CLibDlg::OnQueryDragIcon()
{
...
char* bd_CLibDlg::StripKey(char *c)
{
	char *d;
	int i,k;
...
int bd_CLibDlg::ParseLine(char *s, FILE *fp, CMaterialProp &MProp)
{
		char q[1024];
		char *v;
		int i,k;
...
void bd_CLibDlg::OnBegindragMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void bd_CLibDlg::OnBegindragMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_TREEVIEW* pNMTreeView = (NM_TREEVIEW*)pNMHDR;
	m_dragItem = pNMTreeView->itemNew.hItem;
...
void bd_CLibDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void bd_CLibDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if( m_bIsDragging != FALSE )
    {
		KillTimer ( 1 ) ;
...
BOOL bd_CLibDlg::IsFolder(HTREEITEM hItem)
{
...
HTREEITEM bd_CLibDlg::CopyItemToLibrary(HTREEITEM hItem, HTREEITEM hTarget)
{
...
HTREEITEM bd_CLibDlg::CopyItemToModel(HTREEITEM hItem, HTREEITEM hTarget)
{
	HTREEITEM hResult,hParent,hChild;
...
void bd_CLibDlg::AddNewProperty()
{
...
void bd_CLibDlg::OnRclickMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
		CMenu* pMenu = NULL;
		try
		{
		...
		{
			ASSERT(FALSE);
			TRACE("bd_CLibDlg::OnRclickMytree failed to destroy menu resource\n");
			pMenu = FALSE;
		}
...
void bd_CLibDlg::OnRclickMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
		CMenu* pMenu = NULL;
		try
		{
		...
		{
			ASSERT(FALSE);
			TRACE("bd_CLibDlg::OnRclickMylist failed to destroy menu resource\n");
			pMenu = FALSE;
		}
...
void bd_CLibDlg::OnDblclkMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	POINT point;
	GetCursorPos(&point);
	CPoint ptList(point);
...
void bd_CLibDlg::OnDblclkMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	POINT point;
	GetCursorPos(&point);
	CPoint ptList(point);
...
void bd_CLibDlg::ModifyMaterial(int k) 
{
	bdCMatDlg zDlg;
...
void bd_CLibDlg::CopyItemToDisk(HTREEITEM hItem, FILE *fp)
{
	HTREEITEM hChild;
...
void bd_CLibDlg::ImportMaterials()
{
...
BOOL bd_CLibDlg::IsTailChasing(HTREEITEM hSource, HTREEITEM hDest)
{
	HTREEITEM hItem;
...
void bd_CLibDlg::OnKeydownMylist(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void bd_CLibDlg::OnKeydownMytree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	TV_KEYDOWN* pTVKeyDown = (TV_KEYDOWN*)pNMHDR;
...
void bd_CLibDlg::OnTimer(UINT nIDEvent)
{
	POINT pt ;
    GetCursorPos ( &pt ) ;
    RECT rect ;
