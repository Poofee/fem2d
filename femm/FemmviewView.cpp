...
void CFemmviewView::OnDraw(CDC* pDC)
{
	...
	if((LegendFlag==TRUE) && (DensityPlot!=0))
	{
		...
		if (pDoc->Frequency!=0){
			...
			if (DensityPlot==4) sprintf(cc,"Density Plot: |H|, A/m^2");
			if (DensityPlot==5) sprintf(cc,"Density Plot: |H_re|, A/m^2");
			if (DensityPlot==6) sprintf(cc,"Density Plot: |H_im|, A/m^2");
			...
			if (DensityPlot==1) sprintf(cc,"Density Plot: |B|, Tesla");
			if (DensityPlot==2) sprintf(cc,"Density Plot: |H|, A/m^2");
			...
			// Harmonic problems...
				...
				if(pDoc->ProblemType==0){
					...
					s.Format("|H| = %g A/m",sqrt(u.H1.re*u.H1.re + u.H2.re*u.H2.re
						+u.H1.im*u.H1.im + u.B2.im*u.H2.im));
					outbox+="\r\n"+s;
...
void CFemmviewView::OnMenuPlot() 
{
	...
	if ((EditAction!=1) || (pDoc->contour.GetSize()==0)){
		AfxMessageBox("Cannot create a plot;\nNo contour has been defined");
		return;
	}
...
void CFemmviewView::OnMenuIntegrate() 
{
	...
	if (EditAction==0){
		AfxMessageBox("Cannot integrate;\nNo contour or area\nhas been defined.");
		return;
	}
...
void CFemmviewView::OnViewBHcurves() 
{
	...
	if(i==0){
		AfxMessageBox("No nonlinear materials in this solution");
		return;
	}
