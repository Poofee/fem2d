...
/*#ifdef WINE
	...
	char CommandLine[512];
	char MinAngleArgv[256];
	sprintf(CommandLine,"%striangle.exe",BinDir);
	sprintf(MinAngleArgv,"-q%f",MinAngle);
	int ExitCode = _spawnl(_P_WAIT,CommandLine,CommandLine,"-p","-P",MinAngleArgv,
			"-e","-A","-a","-z","-Q","-I",rootname,NULL);
	if (ExitCode!=0)
	{
		AfxMessageBox("Call to triangle was unsuccessful");
		return FALSE;
