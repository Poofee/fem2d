...
CBHPlot::~CBHPlot()
{
	...
	int i;
	for(i=0;i<NumRows;i++) free(M[i]);
	for(i=0;i<NumCols;i++) free(lbls[i]);
	free(M);
	free(lbls);
	NumRows=0;
	NumCols=0;
...
BOOL CBHPlot::Create(int rows, int cols)
{
	...
	if ((M==NULL) || (lbls==NULL)) flag=FALSE;
	for(i=0;i<rows;i++){
		M[i]=(double *)calloc(cols,sizeof(double));
		if (M[i]==NULL) flag=FALSE;
	}
	for(i=0;i<cols;i++){
		lbls[i]=(char *)calloc(80,sizeof(char));
		if (lbls[i]==0) flag=FALSE;
	}
...
void CBHPlot::MakePlot(CDC *pDC, BOOL logscale)
{
	...
	{
		for(i=1;i<NumPts;i++) Pts[i-1]=Pts[i];
		NumPts--;

		for(i=0,j=0;i<NumRows;i++) if (M[i][0]<Pts[0].re) j++;
		NumRows-=j;
		for(i=0;i<NumRows;i++)
			for(k=0;k<NumCols;k++) M[i][k]=M[i+j][k];
	}

	// if logscale, take the log of H
	if(logscale){
		for(i=0;i<NumRows;i++) M[i][0]=log10(M[i][0]);
		for(i=0;i<NumPts;i++) Pts[i].re=log10(Pts[i].re);
	}
	...
	for(i=1;i<NumRows;i++)
		for(j=1;j<NumCols;j++)
		{
			if (M[i][j]>HiY) HiY=M[i][j];
			if (M[i][j]<LoY) LoY=M[i][j];
		}
	...
	for(i=1;i<NumCols;i++){
		...
		pDC->TextOut((int)(OffsetX+Width+10.),((int) OffsetY)+14*i,lbls[i],strlen(lbls[i]));
		pDC->SelectObject(pOldPen); PlotPen.DeleteObject();
	}
	pDC->SetTextColor(0x00000000);
	pDC->TextOut(200,(int) (OffsetY+Height+30),lbls[0],strlen(lbls[0]));
	...
	for(i=0;i<=k;i++){
		...
		pDC->TextOut(((int) OffsetX)-10,(i*(int)Height)/k +(int) OffsetY-6,s,strlen(s));
	}
	k=(int)((HiX-LoX)/dx);
	if((k<4) && (!logscale)){
		dx/=2.;
	...
	for(i=0;i<=k;i++){
		...
		if(logscale) sprintf(s,"%g",pow((double)10,LoX+((double) i)*dx));
		else sprintf(s,"%.0f",LoX+((double) i)*dx);
		pDC->TextOut((int)(((double) i)*d + OffsetX),(int) OffsetY+(int)Height+10,s,strlen(s));
	}
...
BOOL CBHPlot::ToDisk()
{
	...
	if((NumCols==0) || (NumRows==0)){
		AfxMessageBox("No data to write to disk.");
		return FALSE;
	}
