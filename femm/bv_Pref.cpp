...
static char THIS_FILE[] = __FILE__;
	...
	#define	NameColor  clist[10]
	#define	MaskLineColor  clist[11]
	#define	VectorColor  clist[12]
	#define Color00 clist[13] 
	#define Color01 clist[14] 
	#define Color02 clist[15] 
	#define Color03 clist[16] 
	#define Color04 clist[17] 
	#define Color05 clist[18] 
	#define Color06 clist[19] 
	#define Color07 clist[20] 
	#define Color08 clist[21] 
	#define Color09 clist[22] 
	#define Color10 clist[23] 
	#define Color11 clist[24] 
	#define Color12 clist[25] 
	#define Color13 clist[26] 
	#define Color14 clist[27] 
	#define Color15 clist[28]  
	#define Color16 clist[29] 
	#define Color17 clist[30] 
	#define Color18 clist[31] 
	#define Color19 clist[32] 
	#define Grey00 clist[33] 
	#define Grey01 clist[34] 
	#define Grey02 clist[35] 
	#define Grey03 clist[36] 
	#define Grey04 clist[37] 
	#define Grey05 clist[38] 
	#define Grey06 clist[39] 
	#define Grey07 clist[40] 
	#define Grey08 clist[41] 
	#define Grey09 clist[42] 
	#define Grey10 clist[43] 
	#define Grey11 clist[44] 
	#define Grey12 clist[45] 
	#define Grey13 clist[46] 
	#define Grey14 clist[47] 
	#define Grey15 clist[48] 
	#define Grey16 clist[49] 
	#define Grey17 clist[50]
	#define Grey18 clist[51]
	#define Grey19 clist[52]
...
bvCPref::bvCPref(CWnd* pParent /*=NULL*/)
	: CDialog(bvCPref::IDD, pParent)
{
	//{{AFX_DATA_INIT(bvCPref)
...
void bvCPref::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(bvCPref)
	DDX_Control(pDX, IDC_BV_VECTORPLOT, m_d_vplotlist);
	DDX_Control(pDX, IDC_BV_DENSITYPLOT, m_d_dplotlist);
	DDX_Control(pDX, IDC_BV_EDITACTION, m_d_editlist);
	DDX_Control(pDX, IDC_BV_COLORS, m_d_color);
	DDX_Check(pDX, IDC_BV_GREY, m_d_GreyContours);
	DDX_Check(pDX, IDC_BV_LEGEND, m_d_LegendFlag);
	DDX_Text(pDX, IDC_BV_NCONT, m_d_NumContours);
	DDV_MinMaxInt(pDX, m_d_NumContours, 4, 999);
	DDX_Check(pDX, IDC_BV_ONRELOAD, m_d_ResetOnReload);
	DDX_Check(pDX, IDC_BV_SHOWGRID, m_d_GridFlag);
	DDX_Check(pDX, IDC_BV_SHOWREAL, m_d_ShowAr);
	DDX_Check(pDX, IDC_BV_SHOWPTS, m_d_PtsFlag);
	DDX_Check(pDX, IDC_BV_SHOWMESH, m_d_MeshFlag);
	DDX_Check(pDX, IDC_BV_SNAPGRID, m_d_SnapFlag);
	DDX_Check(pDX, IDC_BV_SMOOTHING, m_d_Smooth);
	DDX_Text(pDX, IDC_BV_LIPOINTS, m_d_LineIntegralPoints);
	DDV_MinMaxInt(pDX, m_d_LineIntegralPoints, 10, 100000);
	DDX_Text(pDX, IDC_BV_PLOTPOINTS, m_d_PlotPoints);
	DDV_MinMaxInt(pDX, m_d_PlotPoints, 10, 100000);
	DDX_Check(pDX, IDC_BV_SHOWMASK, m_d_ShowMask);
	DDX_Check(pDX, IDC_BV_SHOWNAMES, m_d_shownames);
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_BV_NCONT, m_IDC_d_NumContours);
	DDX_Control(pDX, IDC_BV_LIPOINTS, m_IDC_d_LineIntegralPoints);
	DDX_Control(pDX, IDC_BV_PLOTPOINTS, m_IDC_d_PlotPoints);

	
}


BEGIN_MESSAGE_MAP(bvCPref, CDialog)
	//{{AFX_MSG_MAP(bvCPref)
	ON_BN_CLICKED(IDC_BV_MODBTN, OnModBtn)
	ON_BN_CLICKED(IDC_BV_RESET, OnResetColors)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// bvCPref message handlers

void bvCPref::OnModBtn() 
{
    CColorDialog dlg;
	COLORREF cclist[16];
    int i,k;
...
BOOL bvCPref::PreTranslateMessage(MSG* pMsg)
{
...
{		
	...
	FILE *fp;
	CString fname = ((CFemmApp *)AfxGetApp())->GetExecutablePath() + "belaview.cfg";
...
void bvCPref::ScanPrefs()
{
	...
	if (fp!=NULL)
	{
		...
		while (fgets(s,1024,fp)!=NULL)
		{
			...
			if( _strnicmp(q,"<VectorColor>",14)==0)
			{
			  v=StripKey(s);
			  sscanf(v,"%i	%i	%i",&cr,&cg,&cb);
