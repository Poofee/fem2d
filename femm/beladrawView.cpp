	...
	if (EditAction==0)
	{
		...
		if(i>=0){
			...
			for(j=0;j<pDoc->circproplist.GetSize();j++)
			{
				...
				{
					sprintf(s,"%s",pDoc->nodelist[i].InConductor);
				}
			}
			ss+=s;
	...
	if (EditAction==1)
	{
		...
		if(i>=0){
			...
			for(j=0;j<pDoc->circproplist.GetSize();j++)
			{
				...
				{
					sprintf(s,"%s",pDoc->linelist[i].InConductor);
				}
			}
			ss+=s;
	...
	if (EditAction==3)
	{
		...
		if(i>=0){
			...
			for(j=0;j<pDoc->circproplist.GetSize();j++)
			{
				...
				{
					sprintf(s,"%s",pDoc->arclist[i].InConductor);
				}
			}
			ss+=s;
...
void CbeladrawView::OnMenuAnalyze() 
{
	...
	sprintf(CommandLine,"\"%sbelasolv.exe\" %s",BinDir,rootname);
