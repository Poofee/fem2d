...
#define CatchNullDocument(); if (pBelaviewDoc==NULL)\
...
int CbelaviewDoc::lua_showdensity(lua_State * L)
{
	...
	n=lua_gettop(L); if(n<3) return 0;
	...
	if ((PlotType>2)|| (PlotType<0)) PlotType=0;
...
PBITMAPINFO CbelaviewDoc::CreateBitmapInfoStruct(HWND hwnd, HBITMAP hBmp)
{
    ...
    if (!GetObject(hBmp, sizeof(BITMAP), (LPSTR)&bmp))
        ...
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy9");
        }
...
void CbelaviewDoc::CreateBMPFile(HWND hwnd, LPTSTR pszFile, PBITMAPINFO pbi,
                  HBITMAP hBMP, HDC hDC)
 {
    ...
    if (!lpBits)
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy10");
//              errhandler("GlobalAlloc", hwnd);
    ...
    //    errhandler("GetDIBits", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy11");
    ...
    {//    errhandler("CreateFile", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy12");
    ...
    if (!WriteFile(hf, (LPVOID) &hdr, sizeof(BITMAPFILEHEADER),
        (LPDWORD) &dwTmp,  NULL))
    {
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy13");
    ...
    if (!WriteFile(hf, (LPVOID) pbih, sizeof(BITMAPINFOHEADER)
                  ...
                  (LPDWORD) &dwTmp, ( NULL)))
//        errhandler("WriteFile", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy14");
...
//           errhandler("WriteFile", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy15");

    // Close the .BMP file.
     if (!CloseHandle(hf))
//           errhandler("CloseHandle", hwnd);
        AfxMessageBox("Critical error on getting bmp info, possible page fault ahoy16");
...
int CbelaviewDoc::lua_selectconductor(lua_State *L)
{
	...
	for(i=0;i<thisDoc->nodelist.GetSize();i++)
		if (thisDoc->nodelist[i].InConductor==j) thisDoc->nodelist[i].ToggleSelect();
	for(i=0;i<thisDoc->linelist.GetSize();i++)
		if (thisDoc->linelist[i].InConductor==j) thisDoc->linelist[i].ToggleSelect();
	for(i=0;i<thisDoc->arclist.GetSize();i++)
		if (thisDoc->arclist[i].InConductor==j) thisDoc->arclist[i].ToggleSelect();
