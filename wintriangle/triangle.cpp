...
void createeventheap(struct event ***eventheap, struct event **events, 
...
long reconstruct(char *elefilename, char *areafilename, char *polyfilename, FILE *polyfile)
...
int formskeleton(FILE *polyfile, char *polyfilename)
...

{
  ...
  if (nonodewritten || (noiterationnum && readnodefile)) {
    ...
    writenodes(&out->pointlist, &out->pointattributelist,
               &out->pointmarkerlist);
#else /* not TRILIBRARY */
    writenodes(outnodefilename, __argc, __argv);      /* Numbers the points too. */
#endif /* TRILIBRARY */
  }
  if (noelewritten) {
    if (!quiet) {
...
#else /* not TRILIBRARY */
    writeelements(outelefilename, __argc, __argv);
#endif /* not TRILIBRARY */
  }
  ...
  if (geomview) {
    writeoff(offfilename, __argc, __argv);
  }
#endif /* not TRILIBRARY */
  if (edgesout) {
#ifdef TRILIBRARY
...
#else /* not TRILIBRARY */
    writeedges(edgefilename, __argc, __argv);
#endif /* not TRILIBRARY */
  }
  if (voronoi) {
#ifdef TRILIBRARY
...
#else /* not TRILIBRARY */
    writevoronoi(vnodefilename, vedgefilename, __argc, __argv);
#endif /* not TRILIBRARY */
  }
  if (neighbors) {
#ifdef TRILIBRARY
...
#else /* not TRILIBRARY */
    writeneighbors(neighborfilename, __argc, __argv);
#endif /* not TRILIBRARY */
  }
