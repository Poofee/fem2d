...
sure the mesh is up to date before running an analysis."
...
"COGetProblemInfo[] returns a list with two values: the Problem
Type (0 for planar and 1 for axisymmetric) and the depth assumed for planar problems
in units of meters."
...
"HOGetProblemInfo[] returns a list with two values: the Problem
Type (0 for planar and 1 for axisymmetric) and the depth assumed for planar problems
in units of meters."
...
"EOGetProblemInfo[] returns a list with two values: the Problem
Type (0 for planar and 1 for axisymmetric) and the depth assumed for planar problems
in units of meters."
...
"MIAddBoundProp[\"propname\",A0,A1,A2,Phi,Mu,Sig,c0,c1,BdryFormat]
...
"MOGetProblemInfo[] returns a list of two values: the problem type (0 for
planar, 1 for axisymmetric) and the analysis frequency in Hz."
...
MOSmoothOff::usage = "MOSmoothOff[] turns off smoothing of the D and E fields.  The D and E fields are then
displayed using values that are piecewise constant over each element."

MOSmoothOn::usage = "MOSmoothOn[] turns on smoothing of the D and E fields.  The D and E fields are then
displayed using a linear interpolation of the field from nodal values"
...
FEMMPath = "c:\\progra~1\\femm42\\bin\\femm.exe";
...
OpenDebug[] := 
	If[mlink === Null,
        mlink = LinkLaunch["c:\\femm42\\debug\\femm.exe"];
        LinkWrite[mlink, "print(\"MathLink Connected\")"];
		LinkWrite[mlink, "setcompatibilitymode(0)"],
        Print["already connected!"];
		LinkWrite[mlink, "setcompatibilitymode(0)"]
...
MICreateMesh[]:=MLPut["mi_createmesh()"]
...
MOSmoothOff[]:=MLPut["mo_smooth(\"on\")"]
...
EICreateMesh[]:=MLPut["ei_createmesh()"]
...
EOSmoothOff[]:=MLPut["eo_smooth(\"on\")"]
...
HICreateMesh[]:=MLPut["hi_createmesh()"]
...
HOSmoothOff[]:=MLPut["ho_smooth(\"on\")"]
...
CICreateMesh[]:=MLPut["ci_createmesh()"]
...
CIAddMaterial[name_,ex_,ey_,qv_]:=
        MLPut["ci_addmaterial(" <> QuoteC[name] <>
                NumC[ex] <> NumC[ey] <> Num[qv] <> ")" ]

CIAddMaterial[name_,ex_]:=
        MLPut["ci_addmaterial(" <> QuoteC[name] <>
                Num[ex] <> ")" ]

CIAddMaterial[name_,ex_,ey_]:=
        MLPut["ci_addmaterial(" <> QuoteC[name] <>
                NumC[ex] <> Num[ey] <> ")" ]
...
COSmoothOff[]:=MLPut["co_smooth(\"on\")"]
