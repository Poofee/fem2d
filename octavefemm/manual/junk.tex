...
\item {\tt hi\_drawline(x1,y1,x2,y2)} draws a line from {\tt (x1,y1)} to {\tt (x2,y2)}.  This
eliminates the need to add the endpoints separately and then add a segment linking the endpoints.  This macro
does all three steps with one command.

\item {\tt hi\_drawarc(x1,y1,x2,y2,angle,maxseg)}is the arc segment equivalent of {\tt hi\_drawline}.

\item {\tt hi\_drawpolyline(p)} draws a line with multiple segments.  The points through which the 
polyline passes are described by a matrix with $n$ rows and two columns, {\em e.g.} 
{\tt [0,0;0,1;1,1]}.

\item {\tt hi\_drawpolygon(p)} is similar to {\tt hi\_drawpolyline}, except that the first and last points are
linked by a line to create a closed contour.

\item {\tt hi\_drawrectangle(x1,y1,x2,y2)} draws a rectangle with corners at {\tt (x1,y1)} and {\tt (x2,y2)}.
...
\begin{itemize}
\item \texttt{hi\_addmaterial("materialname", kx, ky, qv)} adds a new material with
...
\item \texttt{ho\_showdensityplot(legend,gscale,upper,lower,type)} Shows the
